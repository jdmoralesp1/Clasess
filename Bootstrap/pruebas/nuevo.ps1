...
$Path              =  "C:\IaC\Colaboracion\CheckList_Servidores_IaC"
$Path_html         =  "C:\IaC\Colaboracion\CheckList_Servidores_IaC\Resultados_CheckList"
...
[Environment]::GetEnvironmentVariable("password", "$passwordcorreo", "Machine")
...
function WriteTeams {
    ...
    $parameters = @{
        "URI"         = 'https://intergrupoig.webhook.office.com/webhookb2/b1ff607f-1511-4437-8a25-cdc93e93ef5c@7fbe498c-e540-4556-b198-7b2e16bff9b0/IncomingWebhook/44afa251460d4480aa996cfa25865c67/e1378b7b-0617-483d-81d8-c67ff258b6ac'
        "Method"      = 'POST'
...
#======================INICIO LOG ================================#
Start-Transcript -Path "C:\IaC\Colaboracion\CheckList_Servidores_IaC\Logs\Checklist_IIS_IaC-$dateUpload.txt"
################################################################################################################################################
...
foreach ($server in $serverlist){
    ...
    ping $server > C:\IaC\Colaboracion\CheckList_Servidores_IaC\ping.txt
    [String]$domincol = "colmedica.local"
    [String]$domincolm = "colmena-salud.com.co"
    if(Select-string -Path "C:\IaC\Colaboracion\CheckList_Servidores_IaC\ping.txt" -Pattern "$domincol" -Quiet){
        writetitleservers $freeSpaceFileName
        $ArrayServicios = Get-Service -Name "*SQL*","ClusSvc","MsDtsServer*","SSISTELEMETRY*" -Exclude "*WID*" -ErrorAction SilentlyContinue -ComputerName $server
    ...
    }elseif (Select-string -Path "C:\IaC\Colaboracion\CheckList_Servidores_IaC\ping.txt" -Pattern "$domincolm" -Quiet){
        writetitleservers $freeSpaceFileName
        $usuario = [System.Environment]::GetEnvironmentVariable("usur","Machine")
        $passwor = [System.Environment]::GetEnvironmentVariable("passwo","Machine")

        $pw = convertto-securestring -AsPlainText -Force -String "$passwor"
        $cred = new-object -typename System.Management.Automation.PSCredential -argumentlist "$usuario",$pw
        $ArrayServicios = Invoke-Command -ComputerName $server -Credential $cred -ScriptBlock{
                Get-Service -Name "*SQL*","ClusSvc","MsDtsServer*","SSISTELEMETRY" -Exclude "*WID*" -ErrorAction SilentlyContinue
            }
        $conteo = $ArrayServicios.Count
        if($conteo -eq 0){
            #funcion uptime $uptime
            [string]$uptime = Invoke-Command -ComputerName $server -Credential $cred -ScriptBlock{
            ...
            #funcion KB
            #$variable = Get-HotFix -ComputerName $server | select HotFixId 
            $variable =  Invoke-Command -ComputerName $server -Credential $cred -ScriptBlock {
                (Get-HotFix | Sort-Object -Property InstalledOn)[-10] | select HotFixId
            }
            #$variable = Get-HotFix | select HotFixId 
            $kb = Out-String -InputObject $variable.HotFixId
    ...
    Remove-Item C:\IaC\Colaboracion\CheckList_Servidores_IaC\ping.txt
...
[Environment]::SetEnvironmentVariable("passwo", "", "Machine")
[Environment]::SetEnvironmentVariable("usur", "", "Machine")
